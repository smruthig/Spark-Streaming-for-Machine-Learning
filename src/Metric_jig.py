...
def naive_bayes(X, y, test_X, test_y):
	...
	#WHAT DOES IT DO?
	global count             #count varibale to check for the last batch for metrics func call
	count=count+1
	...
	if count==3 or count==1:    #dummy batch numbers to check results instantly. True check value should be 300
		metrics_score(test_y,pred)
		#np.savetxt(f, metrics_score(test_y,pred))		
	#return(pred)
...
def sgd_class(X, y, test_X, test_y):
	...
	global count
	if count==3 or count==10:    #dummy batch numbers to check results instantly. True check value should be 300
		metrics_score(test_y,pred)
	#ACCURACY
...
def test_fn():
	test_data = pd.read_csv('/home/pes1ug19cs197/Desktop/bd/test.csv')
...
def metrics_score(y_true, y_pred):
	...
	print("Confusion matrix , accuracy , precision , F1 , Recall")
	print(conf_matrix,accuracy,precision,f1,recall)
	#arrayy = np.array([accuracy,precision,f1,conf_matrix,recall])
	#return arrayy
